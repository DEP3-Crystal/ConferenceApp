...
import al.crystal.conferenceApp.validator.service.*;
...
public class ConferenceAppApplication implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        List<Track> tracks = trackService.saveListOfTracks(trackDTOS);
        List<Speaker> speakers = fakerDataAccess.speakerList(3);
        System.out.println(speakers);
        List<Speaker> speakers1 = speakerService.saveAllSpeakers(speakers);
        List<SessionDTO> sessionDTOS = fakerDataAccess.sessionList(3, event1, tracks, speakers1);
//        List<Session> sessions = sessionService.saveSessionList(sessionDTOS);
