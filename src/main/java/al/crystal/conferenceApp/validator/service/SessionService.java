...
//                .sessionSpeakers(sessionDTO.getSpeakers())
                .build();
        Session sessions = sessionRepository.save(newSession);
        SessionSpeaker sessionSpeaker = new SessionSpeaker(sessions, sessionDTO.getSpeakers().stream().findFirst().orElseThrow());
        sessions.getSessionSpeakers().add(sessionSpeaker);
...
//        Sort.by("startTime")
    ...
    public List<Session> saveSession(List<SessionDTO> list){
        list.stream().map(sessionDTO->Session.builder()
                .description(sessionDTO.getDescription())
                .endTime(sessionDTO.getEndTime())
                .startTime(sessionDTO.getStartTime())
                .title(sessionDTO.getTitle())
